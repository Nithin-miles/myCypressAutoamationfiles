 ...
 -->
  ...
  <div>
    ...
    <a-layout class="filesUpload" :style="height()">
      ...
      >
        ...
        <div class="treeContainer">
          ...
          >
            ...
            <a-icon slot="windows" type="windows" />
            <a-icon slot="bank" type="bank" />
            <a-icon slot="pushpin" type="pushpin" />
            <a-icon slot="dingding" type="dingding" />
      ...
      <a-layout  class="box_left" v-if="selectPageKey != '0-4-3'">
        <a-layout-header class="toolbar" v-show="false">
          <a-row>
            <a-col
              :span="11"
              :xxl="18"
            ...
            >
              ...
              <a-button type="link" @click="fileDown()">
        ...
        <a-layout-header class="breadcrumb" v-show="false">
          <a-row>
            <a-col
              :span="19"
              :xxl="18"
                  ...
                  <span>
                    ...
                    <span v-if="record.fileType == 'folder'" >
                      <a style="cursor: pointer">
                        ...
                        />
                       <a-tooltip class="text-overflow" v-if="record.fileName.length>maxLength" :title="record.fileName" placement="topLeft">
  ...
  <share-files :filesShareVisible.sync="filesShareVisible"></share-files> -->
    ...
    >
      ...
      <a-tabs default-active-key="1" type="card" class="tab">
        ...
        >
          ...
          >
            <div slot="title">
              <span v-if="index == 0">{{ item.createUser }}</span>
              <span v-else>
                <a-tag color="#3dbd7d" style="font-weight: 600; color: white"
                  >V{{ reverseNum[index] }}</a-tag
            ...
            <a-row type="flex" justify="space-around" align="middle">
              <a-col :span="12">
                <a-tag color="blue" v-if="item && index == 0"> 当前版本 </a-tag>
                <span v-else style="over-flow: hidden">{{
                  item.fileDesc
                }}</span>
              </a-col>
              <a-col :span="12" type="flex" justify="space-around" align="end">
                大小:
                <span>
                  {{ item.fileStorage | KBDate(item.fileStorage) }}
                  &nbsp;</span
                >
                <a-dropdown v-if="index != 0">
                  ...
                  ></a-button>
                  <a-menu slot="overlay">
                    <a-menu-item @click="openUpFile(item)">
                      <a href="javascript:;">
                    ...
                    <a-menu-item @click="setVersion(item)">
                      <a href="javascript:;">
                        <a-icon type="menu" /> 设置为当前版本</a
                      >
                    </a-menu-item>
                    <a-menu-item @click="deletHistory(item)">
                      <a href="javascript:;">
                        <a-icon type="delete" /> 删除该版本</a
                      >
                    </a-menu-item>
                    <a-menu-item @click="deletAllHistory">
                      <a href="javascript:;">
...
export default {
  ...
  props: {
    ...
    uploadHttp: {
      //上传
      type: String,
      default: "http://172.17.2.169:6001",
    },
    queryFileHttp: {
      //文件验证
      type: String,
    ...
    token: {
      type: String,
      default: "89f82ce4-76cc-4819-ac76-10088d90fa46",
    ...
    config: {
      ...
      default() {
        return {
          ...
          panelControl:true,  //true 
          select: "radio", //单选radio 多选checkbox
          disableSelect: {
            //禁用选择
            state: false, //禁用选择
  ...
  mounted() {
    ...
    oBtnClose.onclick = function () {
      localStorage.setItem("upLoadModel", "false")
      oDiv.style.display = "none";
    ...
    //保存弹窗 状态
    if (localStorage.getItem("upLoadModel") != null) {
	    let upLoadModel=localStorage.getItem("upLoadModel")
      if (upLoadModel=="true"){
         oDiv.style.display = "block";
      }else{
         oDiv.style.display = "none";
  ...
  methods: {
        ...
        .then((res) => {
          ...
          if (res.data.code == 200) {
            ...
            if (res.data.data.fileVirtualHistoryVersionVO != null) {
              if (
                res.data.data.fileVirtualHistoryVersionVO.historyVersionCount !=
                null
              ) {
                that.fileHistoryTotal =
                  res.data.data.fileVirtualHistoryVersionVO.historyVersionCount;
              }
            }
            ...
            if (
              res.data.data.fileVirtualHistoryVersionVO
                .fileVirtualHistoryVersionList != null
            ) {
              for (
                let i = 0;
                i <
                res.data.data.fileVirtualHistoryVersionVO
                  .fileVirtualHistoryVersionList.length;
                i++
              ) {
                that.fileHistoryList.push(
                  res.data.data.fileVirtualHistoryVersionVO
                    .fileVirtualHistoryVersionList[i]
                );
              }
            }

            if (that.fileHistoryList.length > 0) {
              that.fileAttributeUploadID = that.fileHistoryList[0].id;
            }
    ...
    upVersion(dataID, fileName) {
      ...
      axios
        .get(
          this.serviceHttp +
            "/cloudoffice/fileAttributes/addFileVersion?id=" +
            fileID +
            "&currentVersionId=" +
            abc,
          // data,
    ...
    navOnSelect(selectedKeys, info) {
      ...
      } else if (selectedKeys == "0-5-0") {//图片
         console.log("图片")
      }else if (selectedKeys == "0-5-1") {//文档
         console.log("文档")
      }else if (selectedKeys == "0-5-2") {//音视频
         console.log("音视频")
      }else if (selectedKeys == "0-5-3") {//压缩包
         console.log("压缩包")
      }else if (selectedKeys == "0-5-4") {//矢量文件
         console.log("矢量文件")
      }else if (selectedKeys == "0-5-5") {//栅格文件
         console.log("栅格文件")
      }else if (selectedKeys == "0-5-6") {//栅格文件
         console.log("数据文件")
      }else if (selectedKeys == "0-5-7") {//其它
         console.log("其它")
      }
      ...
      this.pageNum = 1;
      this.selectPageKey = selectedKeys;//用于回收站
      if (selectedKeys == "0-0") {
        //如果点击收藏页面
        this.CollectPage();
      } else {
    ...
    fileDown(e) {
      //index 接口参数  fileName文件名字
      let that = this;
      let data = {
        fileVirtualFolderInfoVOList: [],
        fileVirtualInfoVOList: [],
      };
      console.log(this.shareData);
      this.shareData.forEach((item) => {
        if (item.isFolder == 1) {
          data.fileVirtualFolderInfoVOList.push({ id: item.id });
        } else {
          data.fileVirtualInfoVOList.push({ id: item.id });
        }
      });
      console.log(data);
      ...
      this.$http({
        ...
        url:
          that.serviceHttp +
          "/cloudoffice/fileVirtualFolderInfo/downLoadFileAndFolder",
        data,
        headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + that.dataToken,
    ...
    filesUploadModal() {
      ...
      var oDiv = document.getElementById("miaov_float_layer");
      oDiv.style.display = "block";
      localStorage.setItem("upLoadModel", "true")
    },
    ...
    onSelectTable(record, selected, selectedRows){
      ...
      //  this.selectedRowKeys=[]
       if(this.config.select=="checkbox"){
         console.log("checkbox")
       }else{
         console.log("Radio")
          // if(this.selectedRowKeys.length>1){
          //     this.selectedRowKeys=[]
          //     this.selectedRowKeys=selectedRows[selectedRows.length-1]
          // }
    ...
    attributeHandleOk(e) {
      ...
      this.setFileNameAndDesc(this.fileContent);
      // setTimeout(() => {
    ...
    //文件属性修改文件名和描述
    setFileNameAndDesc(record) {
      let that = this;
      let data = {
        id: record.id,
        fileDesc: that.fileAttributeForm.filedesc,
      };
      let http = "";
      if (record.isFolder == 1) {
        data.folderName = that.fileAttributeForm.fileName;
        http = "/cloudoffice/fileVirtualFolderInfo/updateFileVirtualFolderInfo";
      } else {
        data.fileName = that.fileAttributeForm.fileName;
        http = "/cloudoffice/fileVirtualFolderInfo/updateFileVirtualInfo";
      }
      console.log(data);
      axios
        .put(that.serviceHttp + http, data, {
          headers: {
            "Content-Type": "application/json",
            Authorization: "Bearer " + that.dataToken,
          },
        ...
        .then((res) => {
          if (res.data.code == 200) {
            ...
            that.attributeLoading = false;
            that.attributeVisible = false;
            that.fileList();
          }
        })
        .catch((error) => {
    ...
    onSelectChange(selectedRowKeys, selectedRows) {
      ...
      this.$emit('selectRadio',selectedRows[selectedRows.length-1])

      this.$emit('changeCheckbox',selectedRows)
    ...
    setVersion(record) {
      ...
      console.log(record, data, "____________--");
      axios
        .put(
          this.serviceHttp +
            "/cloudoffice/fileAttributes/setHistoryVersionToCurrent",
          data,
          {
            headers: {
              "Content-Type": "application/json",
        ...
        .then((res) => {
          ...
          if (res.data.code == 200) {
            ...
            this.fileAttributeUploadID = record.id;
            this.attributeFileList(record);
          }
        })
        .catch((error) => {
          console.log(error);
    ...
    deletHistory(record) {
      ...
      axios
        .post(
          this.serviceHttp +
            "/cloudoffice/fileAttributes/deleteSingleFileVersion",
          data,
          {
            headers: {
              "Content-Type": "application/json",
    ...
    deletAllHistory() {
      let data = {
        id: this.fileAttributeUploadID,
      };
      axios
        .post(
          this.serviceHttp +
            "/cloudoffice/fileAttributes/deleteAllHistoryVersion",
          data,
          {
            headers: {
              "Content-Type": "application/json",
  ...
  watch: {
    ...
    fileHistoryList(val) {
      ...
      this.reverseNum = newArr.reverse();
      console.log(newArr, "===+++====");
    },
    searchForm: {
